...
float* matrix_multiplication_openblas(float* matrix1,int m1,int n1,float* matrix2,int m2,int n2);
...
int main() {
  ...
  for(int k=0;k<sizes.size();k++) {
    ...
    for(int i=0;i<noofvalues;i++) {
      ...
      float* result=matrix_multiplication_openblas(matrix,sizes[k],kernel_m,kernel,kernel_m,kernel_n);
    ...
    //writing to file
    string concat="openblas"+to_string(sizes[k])+".txt";
